...
std::string ParseWord( const std::string& p_string, int p_index, const std::string separator )
{
    int wss = p_string.find_first_not_of( separator );
    ...
    while( p_index > 0 )
    {
        ...
        // of the next word
        wss = p_string.find_first_of( separator, wss );
        wss = p_string.find_first_not_of( separator, wss );
    }

    // find the end of the word
    int wse = p_string.find_first_of( separator, wss );
...
std::string RemoveWord( const std::string& p_string, int p_index, const std::string separator )
{
    int wss = p_string.find_first_not_of( separator );
    ...
    wse = p_string.find_first_not_of( separator, wse );
    ...
    std::string str = std::string(p_string.c_str(), p_string.size());
...
bool DirExists(const char * pzPath)
{
	...
	if (_stat(pzPath, &fileStat) == 0)
	{
		return fileStat.st_mode & _S_IFDIR;
	}
