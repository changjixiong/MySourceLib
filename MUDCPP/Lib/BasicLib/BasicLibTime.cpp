...
namespace BasicLib
{
	...
	std::string DateTimeStampShort()
    {
        ...
        // print the time to the string
        strftime( str, 20, "%Y%m%d%H%M%S", b );		
		
        return str;
    }

	std::string CurrentTime(long nLen)
	{
	...
	void DeBugOut(const std::string &strMsg)
	{
		ofstream fileout;
		cout<<CurrentTime().c_str()<<" "<<strMsg<<endl;

		fileout.open("fileout.txt",ios::app);	
		fileout<<CurrentTime().c_str()<<":"<<strMsg<<endl;
		fileout.flush();
