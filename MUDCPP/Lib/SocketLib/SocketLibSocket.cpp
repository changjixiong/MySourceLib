...
namespace SocketLib
{
    ...
    int DataSocket::Send( const char* p_buffer, int p_size )
    {
        ...
        {
			BasicLib::DeBugOut("Send ENotConnected");
        ...
        {
            Error e = GetError();
			BasicLib::DeBugOut("Send EOperationWouldBlock");
            if( e != EOperationWouldBlock )
            {
				BasicLib::DeBugOut("Send error e!=EOperationWouldBlock");
                throw Exception( e );
            }
    ...
    int DataSocket::Receive( char* p_buffer, int p_size )
    {
        ...
        if( m_connected == false )
        {
			BasicLib::DeBugOut("Receive error m_connected is false");
            throw Exception( ENotConnected );
        }
        ...
        if( err == 0 )
        {
			BasicLib::DeBugOut("Receive error err == 0");
            throw Exception( EConnectionClosed );
        }
        if( err == -1 )
        {
			BasicLib::DeBugOut("Receive error err == -1");
            throw Exception( GetError() );
        }
    ...
    void ListeningSocket::Listen( port p_port )
    {
        ...
        if( m_sock == -1 )
        {
            ...
            if( m_sock == -1 )
            {
				BasicLib::DeBugOut("socket init error");
                throw Exception( GetError() );
            }
        }
        ...
        if( err != 0 )
        {
			BasicLib::DeBugOut("setsockopt error");
            throw Exception( GetError() );
        }
        ...
        if( err == -1 )
        {
			BasicLib::DeBugOut("listen error");
            throw Exception( GetError() );
        }
    ...
    DataSocket ListeningSocket::Accept()
    {
        ...
        if( s == -1 )
        {
			BasicLib::DeBugOut("Accept error");
            throw Exception( GetError() );
        }
