...
public:
    ...
    //              socket is accepted.
    // ====================================================================
    void SetConnectionManager( ConnectionManager* p_manager );
...
void ListeningManager::
SetConnectionManager( ConnectionManager* p_manager )
{
...
void ListeningManager::Listen()
{
	...
	int nActive;
    if( (nActive = m_set.Poll()) > 0 )
    {
		char szMsg[128];
		sprintf(szMsg, "%d connection comes", nActive);
		BasicLib::DeBugOut(szMsg);
